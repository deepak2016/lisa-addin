(function () {
   ...
   */
    function homeController(dataService, officeAddinService, utilitiesService) {
        ...
        vm.emojifymail = function emojifymail() {
            officeAddinService.getBodyContent()
                .then(function (content) {
                emojifycontent(content.value);
            }, function (error) {
                vm.state.emoji = getError("Error getting body content: " + error.message);
                console.log(error, error.message);
            });
        }

        function emojifycontent(content) {
            if (!emojify) {
                console.log("cant find emojify")
                return;
            }
            var emojifiedStr = emojify.replace(content);
            console.log(emojifiedStr);
            officeAddinService.setBodyContentHTML(emojifiedStr);
        }    
